...
def pytest_generate_tests(metafunc):
    ...
    if 'before-results' in metafunc.fixturenames and option_value is not None:
        metafunc.parametrize('--before-results', [option_value])

    option_value = metafunc.config.option.after_results
    if 'after-results' in metafunc.fixturenames and option_value is not None:
        metafunc.parametrize('--after-results', [option_value])
